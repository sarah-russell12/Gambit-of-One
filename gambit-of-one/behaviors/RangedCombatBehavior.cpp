...
RangedCombatBehavior::RangedCombatBehavior(Creature& node, const CreatureData& data)
	: CombatBehavior(node, data), mIsFiring(false)
{
	mAttackInterval = sf::Time::Zero;
	mAttackCooldown = 3.f * data.attackInterval;
}
